/***************************************************
  This is the programm of a connected recycle bin.
 ****************************************************/
...
#define BUTTON_TRASH_X    (SCREEN_WIDTH/2)+MARGIN
#define BUTTON_TRASH_Y    BANNER_HEIGHT+MARGIN
#define BUTTON_RECYCLE_X  MARGIN
...
void draw_buttons(status_t status)
{
  ...
  for(uint8_t j=0; j<BUTTON_HEIGHT; j++) {
    for(uint8_t i=0; i<BUTTON_WIDTH; i++){
      if((i%2==0) && (j%2==0)){
        tft.drawPixel(BUTTON_TRASH_X + i,BUTTON_TRASH_Y + j, COLOR_GREY);
      }
    }
  }
}
